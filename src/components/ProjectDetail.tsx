...
const projectsData: Record<string, any> = {
  'ecommerce-ai': {
    ...
    client: 'E-Shop Pro',
    challenge: 'Baixa taxa de conversão e dificuldade em recomendar produtos relevantes',
    solution: 'Sistema de recomendação por IA que analisa comportamento em tempo real',
...
export function ProjectDetail({ projectId, onNavigate }: ProjectDetailProps) {
  ...
  const [animatedResults, setAnimatedResults] = useState(project.results.map(() => 0));

  useEffect(() => {
    project.results.forEach((result: any, index: number) => {
      const duration = 2000;
      const steps = 60;
      ...
      const interval = setInterval(() => {
        ...
        if (currentStep <= steps) {
          setAnimatedResults((prev) => {
            ...
            newResults[index] = Math.min(increment * currentStep, result.after);
            return newResults;
          });
        } else {
      ...
      <div className="container mx-auto px-4 md:px-6 pb-20">
        ...
        >
          <h2
            ...
            style={{ fontFamily: 'Plus Jakarta Sans, sans-serif', fontWeight: 800 }}
          >
            Resultados e <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-500 to-violet-500">Impacto</span>
          </h2>
                    ...
                    <div>
                      ...
                      <div className="flex items-baseline gap-3">
                        ...
                        >
                          ...
                          {animatedResults[index].toFixed(result.unit === 'min' ? 1 : 0)}
                          {result.unit !== 'R$' && result.unit}
                        </span>
                        {improvement > 0 && (
